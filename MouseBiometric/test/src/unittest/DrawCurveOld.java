...
public class DrawCurveOld {
	...
	static public void main(String[] args){
        ...
        for (int j = 0; j<45; ++j) I.setRGB(j, j, 0); // Scratch upper left corner
        WritableRaster wr = I.getRaster();
        if(1>0) {int x = X-40, y = Y-40; int[] a = new int[y*x*3]; // 96 bit pixels
          for(int j = 0; j<y; ++j) for(int i = 0; i<x; ++i) {int z = 3*(j*x + i);
