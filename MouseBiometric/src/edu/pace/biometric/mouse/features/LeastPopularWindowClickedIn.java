...
public class LeastPopularWindowClickedIn extends MouseFeature {
	public LeastPopularWindowClickedIn(MouseLogParser parser, String app) {
		super(parser, app);
	}
	...
	public FeatureResult extract() {
		...
		String leastPopular="Could not calculate";
		int count=Integer.MAX_VALUE;
		for(Map.Entry<String, Integer> me:map.entrySet()){
			if(me.getValue().intValue()<count){
				count=me.getValue();
				leastPopular=me.getKey();
			}
		}
		return new FeatureResult(getClass().getName(), "Least Popular Window Clicked In", leastPopular, "least popular");
	}
}
