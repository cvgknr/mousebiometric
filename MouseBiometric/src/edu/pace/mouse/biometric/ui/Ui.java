...
import edu.pace.mouse.biometric.persist.DataStore;
import edu.pace.mouse.biometric.persist.PersistentStore;
...
public class Ui extends JFrame {
	...
	private MouseUserProfile profile = null;
	private ArrayList<FeatureResult[]> frs=null;
		...
		extractBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				try{
					...
					MouseLogParser parser = new MouseLogParser(fileName);
					profile = parser.getUserProfile();
					String t="<b>User: "+profile.getFirstName()+" "+profile.getLastName()+"</b><br/>";
					frs=FeatureExtractor.extractFeaturesFrom(parser);
					pane.setText(t+createFeatureResultString(frs));
					try{
						DataStore ds=PersistentStore.getDataStore();
						ds.saveFeatures(frs,profile.getFirstName(),profile.getLastName());
		...
		clearUserDataBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if(action==JOptionPane.OK_OPTION){
					...
					try{
						DataStore ds=PersistentStore.getDataStore();
						ds.clearDataForThisUser(profile.getFirstName(), profile.getLastName());
						pane.setText("");
						filechosen.setText("                                                    ");
						label1.setVisible(true);
						exportDataBtn.setEnabled(false);
						clearUserDataBtn.setEnabled(false);
						JOptionPane.showMessageDialog(basic, "Data for "+profile.getFirstName()+" "+profile.getLastName()+" has been deleted", "Information", JOptionPane.INFORMATION_MESSAGE);
					}catch (Exception e) {
						JOptionPane.showMessageDialog(basic, "Error saving to database:"+e.getMessage(), "Information", JOptionPane.INFORMATION_MESSAGE);
					}
					setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
